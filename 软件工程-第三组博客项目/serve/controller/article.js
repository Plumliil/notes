...
exports.getArticles = async (req, res, next) => {
    try {
        ...
        articles.forEach(async item => {
            const author = await User.find({ _id: item.author })
            item.authorName = author[0].username
        })
        if (articles.length === 0) {
            return res.status(200).json({
                message: '这里暂时还没有内容，看点别的吧！',
                empty: true
            })
        }
...
// 获取用户文章
exports.getUserArticle = async (req, res, next) => {
    try {
        const { limit = 20, offset = 0, tag, author } = req.query
        ...
        const userArticles=await Article.find({author:req.params.userId})
            .limit(parseInt(limit)) // 选中多少条
            .skip(parseInt(offset)) // 跳过多少条
            userArticles.forEach(async item => {
                const author = await User.find({ _id: item.author })
                item.authorName = author[0].username
            })
